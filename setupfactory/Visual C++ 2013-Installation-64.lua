-- Check to see if this is a valid operating system for VC 2013 x64
function isValidOS()
	SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Entering compatible OS detection.\r\n",true);
	...
	if ((nMajorVersion < 5) or ((tblOSInfo.MajorVersion == "5") and (nMinorVersion < 2))) then
		SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Operating systems less than Windows XP Professional x64 SP2 are not supported.\r\n",true);
		return false;
	end
	
	-- Check Windows XP Profesional x64 SP2 or later.
	if ((tblOSInfo.MajorVersion == "5") and (tblOSInfo.MinorVersion == "2") and (tblOSInfo.ProductType == 1)) then
		...
		if (tblOSInfo.ServicePackMajor < 2) then
			SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Windows XP Professional x64 SP2+ required.\r\n",true);
			return false;
		else
			-- Windows XP Professional x64 SP2+ acceptable.
			return true;
		end
	end
	
	-- Check Windows Server 2003 x64 SP2 or later
	if ((tblOSInfo.MajorVersion == "5") and (tblOSInfo.MinorVersion == "2") and (tblOSInfo.ProductType ~= 1) and (not tblOSInfo.Server2003R2)) then
		-- Check service pack.
		if (tblOSInfo.ServicePackMajor < 2) then
			SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Windows Server 2003 x64 SP2+ required.\r\n",true);
			return false;
		else
			-- Windows Server 2003 x64 SP2+ acceptable
			return true;
		end
	end
	
	-- Check Windows Server 2003 R2 x64 or later.
	if ((tblOSInfo.MajorVersion == "5") and (tblOSInfo.MinorVersion == "2") and (tblOSInfo.Server2003R2)) then
		-- Windows Server 2003 R2 x64 acceptable.
		return true;
	end
	
	-- Check Windows Vista x64 SP2+ / Windows Server 2008 x64 SP2 or later.
	if ((tblOSInfo.MajorVersion == "6") and (tblOSInfo.MinorVersion == "0")) then
		if (tblOSInfo.ServicePackMajor < 2) then
			SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Windows Vista x64 SP2+ / Windows Server 2008 x64 SP2+ required.\r\n",true);
			return false;
		else
			-- Check to make sure not Starter Edition (even though no x64 Starter Edition)
			if (tblOSInfo.StarterEdition) then
				SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Windows Vista Starter Edition not supported.\r\n",true);
				return false;
			else
	...
	-- Check Windows 7 x64 / Windows Server 2008 R2 x64
	if ((tblOSInfo.MajorVersion == "6") and (tblOSInfo.MinorVersion == "1")) then
		-- Windows 7 / Windows Server 2008 R2 x64 acceptable.
		return true;
	end
...
local strRuntimeSupportFolder = SessionVar.Expand("%TempLaunchFolder%\\vc2013x64");
...
SetupData.WriteToLogFile("Success\tVisual C++ 2013 (64-bit) Module: Installation script started.\r\n", true);
...
-- Windows Vista x64 Service Pack 2 (SP2) or later
-- Windows Server 2008 x64 Service Pack 2 (SP2) or later
-- Windows XP Professional x64 Edition Service Pack 2 (SP2)
-- Windows Server 2003 R2 x64 or later (all editions)
-- Windows Server 2003 x64 Service Pack 2 (SP2) or later (all editions)                                            
------------------------------------------------------------------------------------------------------------
...
if (not isValidOS()) then
	bRequirementFail = true;
	--VC 2013 isn't supported on the x64 OS that was detected.
	strTemp = "- The Visual C++ 2013 runtime cannot be installed on this operating system. Requires Windows 7 x64, Windows Server 2008 R2 x64, Windows Vista x64 SP2+, Windows Server 2008 x64 SP2+, Windows XP Professional x64 SP2+, Windows Server 2003 x64 SP2+, or Windows Server 2003 R2 x64.";
	Table.Insert(tbRequirementFailStrings, Table.Count(tbRequirementFailStrings) + 1, strTemp);
end
...
if (not b64BitOs) then
	bRequirementFail = true;
	-- This version of the VC 2013 redistributable isn't supported on 32-bit OS's.
	strTemp = "- This version of the Visual C++ 2013 runtime cannot be installed on this operating system platform (32-bit).";
	Table.Insert(tbRequirementFailStrings, Table.Count(tbRequirementFailStrings) + 1, strTemp);
end
...
if (not bRequirementFail) then
	...
	if (bRunInstallFile) then
		...
		SetupData.WriteToLogFile("Info\tVisual C++ 2013 (64-bit) Module: Visual C++ runtime installation file "..strExtractInstallerToPath.." is being launched.\r\n");
...
Click OK to abort the setup, or Cancel to continue with the application install.]];
		elseif (nResult == 0) then
			...
			SetupData.WriteToLogFile("Success\tVisual C++ 2013 (64-bit) Module: Visual C++ 2013 runtime installed.\r\n");
		else
...
Click OK to abort the setup, or Cancel to continue with the application install.]];
		...
		if (strMessageFail ~= "") then
			...
			SetupData.WriteToLogFile("Error\tVisual C++ 2013 (64-bit) Module: Dialog error shown- "..strMessageFail..".\r\n");
		...
		if (bAbortInstall) then
			...
			SetupData.WriteToLogFile("Error\tVisual C++ 2013 (64-bit) Module: User chose to abort setup after Visual C++ 2013 install failure. Exiting setup.\r\n");
		...
		SetupData.WriteToLogFile("Error\tVisual C++ 2013 (64-bit) Module: User chose to abort setup. Exiting setup.\r\n");
...
-- A requirement failed
	...
	if (bRunInstallFile) then 
		...
		SetupData.WriteToLogFile("Error\tVisual C++ 2013 (64-bit) Module: Requirement failure dialog: "..strFullErrorString.."\r\n");
		...
		if (nDialogResult == IDOK) then
			...
			SetupData.WriteToLogFile("Error\tVisual C++ 2013 (64-bit) Module: User aborted setup due to requirement failure. Exiting setup.\r\n");
...
SetupData.WriteToLogFile("Success\tVisual C++ 2013 (64-bit) Module: Installation script finished.\r\n");