function isDotNet_Installed()
	...
	if (DotNet_Registry == false) then
		...
		SetupData.WriteToLogFile("Info\t.NET 4 Module: No version of .NET 4.5 was found. .NET 4.5 will be installed.\r\n", true);
		return false;
		...
		if (DotNet_Install_Success == "1") then
			...
			Compare = String.CompareFileVersions(DotNet_Install_Version, "4.5.50938");
							
			if (Compare == 0 or Compare == 1) then
				-- .NET version 4 is installed already
				SetupData.WriteToLogFile("Info\t.NET 4.5 Module: .NET version 4.5 is installed already\r\n", true);
				return true;
			else 	
				SetupData.WriteToLogFile("Info\t.NET 4 Module: A lesser version of .NET 4.5 was found on the users system.\r\n", true);
				return false;
			end
		else
